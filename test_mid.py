...
os.makedirs('demo/middlebury0.008/', exist_ok=True)
...
def test_trainset():
    ...
    for i in trange(len(train_limg)):
        ...
        filename = os.path.join('demo/middlebury0.008/', limg_path.split('/')[-2] + limg_path.split('/')[-1])
        pred_np_save = np.round(pred_np * 256).astype(np.uint16)        
        cv2.imwrite(filename, cv2.applyColorMap(cv2.convertScaleAbs(pred_np_save, alpha=0.008),cv2.COLORMAP_JET), [int(cv2.IMWRITE_PNG_COMPRESSION), 0])
